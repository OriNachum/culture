...
The supervisor uses `opencode --non-interactive` as a subprocess to periodically
evaluate the agent's activity. It runs inside the daemon process and intervenes
minimally when it detects unproductive behavior.

## How It Works

Unlike the Claude backend (which uses a persistent SDK session for the supervisor),
the OpenCode supervisor spawns a fresh `opencode --non-interactive` process for each
evaluation. Each evaluation is a one-shot subprocess:

1. The supervisor formats recent agent turns into a transcript.
2. It spawns `opencode --non-interactive -m anthropic/claude-sonnet-4-6` with an
   isolated HOME environment.
3. The supervisor prompt and transcript are piped to stdin.
4. The subprocess returns a single-line verdict.
5. The process exits and the temporary HOME is cleaned up.

This approach keeps the supervisor stateless between evaluations and avoids
maintaining a long-lived ACP session for monitoring.

## What the Supervisor Watches

The supervisor maintains a rolling window of the last 20 agent turns (completed
assistant messages relayed from the ACP session). Every 5 turns it evaluates the
window and decides whether to act.
...
On step 3, the daemon pauses the agent -- it stops feeding new tasks to the OpenCode
session. The daemon posts a message to IRC `#alerts`:

```text
<spark-opencode> [ESCALATION] Agent spark-opencode appears stuck on task
"benchmark nemotron". Retried same approach 4 times. Awaiting
human guidance. Reply @spark-opencode resume/abort
```
...
```text
@spark-opencode resume
@spark-opencode abort
```
...
Each supervisor evaluation runs with its own isolated HOME directory, just like the
agent runner. This prevents any evaluation from reading or writing the host user's
`~/.config/opencode/` configuration. The temporary HOME is cleaned up after each
